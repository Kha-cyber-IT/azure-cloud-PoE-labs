<div align="center">
  <img src="https://i.postimg.cc/90kYVSYq/Screenshot-from-2026-01-12-16-24-12.png" />
  
  <img src="https://engineer.kodekloud.com/courser/100days_cloud.svg" alt="" width="250" />
  <h1>100 Days of Cloud (Azure)</h1>
</div>
☁️ 100 Days of Azure | Portfolio of Evidence (PoE)

Status:Day 50/100
Curriculum: KodeKloud
Primary Focus: Cloud Administration & Infrastructure

This repository serves as a technical Portfolio of Evidence documenting my journey to master Microsoft Azure. It contains lab reports, architectural notes, and automation scripts.

📊 Competency Breakdown (First 50 Days)
I have structured my learning to align with real-world roles. My first 50 days have been heavily focused on **Cloud Administration** to build a solid infrastructure foundation.

| Role Focus | % of Labs | Key Skills Practiced |
| :--- | :--- | :--- |
| 🟦 Cloud Administrator| 64% | VM Management, VNet Peering, Load Balancers, Storage Lifecycle |
| 🟧 DevOps Engineer | 16%| ARM Templates, Azure CLI Automation, Docker/ACR, AKS |
| 🟥 Security Engineer | 10% | Key Vault, NSGs, SSH Hardening, Private Links |
| 🟨 Data Engineer| 10%| Azure SQL, Event Hubs, Database Migration |

📂 Repository Structure
* Day 01-15: Azure Basics (VMs, Networking, SSH)
* Day 16-20: Storage & ARM Templates
* Day 21-30: Advanced Networking & Databases
* Day 31-40: Web Apps, Containers & Key Vault
* Day 41-50: AKS, Event Hubs & App Gateway

Created by [Kha-cyber-IT](https://github.com/Kha-cyber-IT)
